 ...
 *
 * This project runs Gimplex tested on Supercomputer for LPs with initial Basic Solution
 * as feasible i.e., artificial variables = 0 and it has no Stream implementation in it.
 * The LP dimension that it solves is 510 due to the constraints of 1024 threads available.
 *
 */
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char *argv[]) {
	unsigned int LP_size = 1, avg, dimension, art;
	...
	if (argc > 1) {
		if (argc != 4) { //1(ApplicationName) + 3 (Input Arguments)
			...
			std::cout
					<< "./ProjName --'Dimension'--'Average'--'Batch-size'  --'Streams'!!\n";
			std::cout
			...
			//std::cout<< "argument 4) Artificial-Variables -- Number of negative on RHS of the LP constraints\n";
...
//  ***************************************************************** //
	...
	for (int i = 0; i < max; i++) {
		std::cout << "GLPK: " << resul[i] << " || GPU: " << result[i]
				<< std::endl;
	}
	return 0;
}
